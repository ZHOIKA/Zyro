        ...
        checkForUpdateUseCase()
            .onEach { result ->
                when(result) {
                    ...
                    is Resource.Success -> {
                        val release = result.data ?: Release()
                        val currentVersion = BuildConfig.VERSION_NAME.toVersion()
                        
                        if (release.toVersion() > currentVersion) {
                            _homeState.value = HomeScreenState.LoadingCompleted(release)
                        } else {
                            _homeState.value = HomeScreenState.AlreadyUpdated
                        }
                    }
                    is Resource.Error -> {
                        _homeState.value = HomeScreenState.Error(
                            result.message ?: "Erro ao verificar atualização"
                        )
                    }
                }
            }
...
sealed interface HomeScreenState {
    ...
    @Stable
    data object AlreadyUpdated : HomeScreenState
